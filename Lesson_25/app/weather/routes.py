...
from utils.weather.getting_weather import main as get_weather, APP_ID
...
def index():
    ...
    city = form.city.data
    weather_in_city = get_weather(city, APP_ID)
    if 'error' in weather_in_city:
        flash(f"{weather_in_city['error']} - ({city}).")
        return redirect(url_for('weather.index'))

    flash(f'The weather In city "{city.capitalize()}" today.')
    form.city.data = ''
    return render_template(
        'weather/index.html',
        title='Home page',
...
def show_cities(page=1, qty_per_page=QTY_PER_PAGE):
    """Show all cities."""
    cities = City.select()
